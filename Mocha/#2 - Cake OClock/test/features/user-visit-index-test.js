...
describe('User visits index', () => {
  describe('to post an order', () => {
    ...
    it('pick-up time field labeled correctly', () => {
      ...
      assert.include(parsedHTML, text);
    })
    
    
    // Checks that delivery times are coorect format
    it('display selected hours', () => {
      browser.url('/');
      browser.selectByVisibleText('#select-pickUp', '9:00');
      browser.click('#submit-order');
      browser.url('/');

      assert.include(browser.getText('#pickUp'), '9:00');
    });
